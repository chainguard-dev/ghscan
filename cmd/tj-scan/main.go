...
import (
	...
	"github.com/chainguard-dev/tj-scan/pkg/cache"
	"github.com/chainguard-dev/tj-scan/pkg/logs"
	"github.com/chainguard-dev/tj-scan/pkg/output"
...
func main() {
	...
	existingCache := cache.LoadExistingCache(logger, *cacheFileFlag, *cleanCacheFlag)
					...
					for _, run := range runs {
						...
						go func(run *github.WorkflowRun) {
							...
							func() {
								...
								logText, err := logs.ExtractLogsFromZip(full)
								if err != nil {
									logger.Errorf("Error extracting logs for run %d: %v", runID, err)
									return
								}
								base64Findings, emptyLines, found := logs.ScanLogData(logger, logText, runID, true)
